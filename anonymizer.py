...
def get_result_one(att_trees, data, K=10, L=5):
    ...
    result = Separation_Gen(att_trees[-1], data, K, L)
...
def get_result_dataset(att_trees, data, K=10, L=5):
    ...
    for i in range(1, h+1):
        ...
        if pos > length:
            continue
        result = Separation_Gen(att_trees[-1], data[0:pos], K, L)
        data = copy.deepcopy(data_back)
...
if __name__ == '__main__':
    ...
    elif flag == 'Separation_Gen':
        get_result_one(att_trees, data)
    else:
        print "Usage: python anonymizer [k | l | data |Separation_Gen]"
